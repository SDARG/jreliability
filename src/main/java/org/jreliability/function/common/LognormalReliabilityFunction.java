...
/**
 * The {@code LognormalReliabilityFunction} represents the {@code ReliabilityFunction} of the
 * {@code Lognormal reliabilityFunction}
 * <p>
 * {@code R(x) = 1 - F(x) = 0.5 - 0.5 * (Math.log(x) - mu) / (rho * Math.sqrt(2))} <br>
 * with {@code mu => 0, rho > 0}.
 * <p>
 * The {@code rho} and {@code mu} parameter represent the standard deviation and mean of the variable's natural
 * logarithm.
 * 
...
public class LognormalReliabilityFunction implements ReliabilityFunction {
	...
	/**
	 * Constructs a {@code LognormalReliabilityFunction} with a given {@code mu} and {@code rho}.
	 * 
	 * @param mu
	...
	public LognormalReliabilityFunction(double mu, double rho) {
		...
		this.rho = rho;
		if (!(rho > 0.0) || !(mu >= 0.0)) {
			throw new IllegalArgumentException("LognormalReliabilityFunction: Mu must be greater equal 0, rho must be greater 0.");
		}
	}
	...
	/**
	 * Estimated Error Function (from the Formula 7.1.26 in "Handbook of Mathematical Functions" by M. Abramowitz and I.
	 * A. Stegun.)
	 * 
	 * @param x
	 *            the x value
	...
	private double erf(double x) {
		...
		double y = 1.0 - (((((a5 * t + a4) * t) + a3) * t + a2) * t + a1) * t * Math.exp(-Math.abs(x) * Math.abs(x));
