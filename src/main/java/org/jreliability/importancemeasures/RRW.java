...
/** 
 * The {@link RRW} class calculates the RRW values for coherent and non-coherent systems.
 ...
 */
public class RRW<T> implements TimeDependentImportanceMeasure<T> {
	...
	/**
	 * Returns a {@link RRW} calculator for a specific system with its {@link BDD}
	 ...
	 */
	public RRW(BDD<T> bdd, Transformer<T, ReliabilityFunction> transformer) {
		this.bdd = bdd;
	...
	/**
	 * Calculates the {@link RRW} values for all components at the specified time.
	 * 
	 * Uses equation 4.7 of [Ali17] to calculate the RRW values from the conditional
	 * unreliability of the system if a component works correctly and the nominal system unreliability.
	 * 
	 ...
	 *   	 In [BAGT16] U_i^- corresponds to no failure in a component.
	 *   	 In [Ali17] U_i^+ corresponds to a correctly working component.
	 *   	 They are equivalent.
	 *   
	 * @param time
	 *            Time t at which the RRW values of all components is calculated.
	 *            Must be greater than 0, otherwise an IllegalArgumentException is thrown.
	 *            
	 * @return results
	 * 			  Map of components and their respective RRW values at time t.
	 */
	public Map<T, Double> calculate(double time) {
		...
		for (T var: bdd.getVariables()) {						
			...
			/* System Unreliability at time t with var i working */
			double u_i_plus = 1 - restrictedSystemReliabilityFunction.getY(time);
			
			/* Equation 4.12 of [ABGT17] */
			double rrw = systemUnreliability / u_i_plus;
